...
Initialize FSL before running atlas preparation (replace the installation path):
...
7. Use `07_run_rapidtide.py select-cohort --subjects <task-subjects.tsv>
   ...
   For `summarize --targets`, use `source_targets.tsv` generated by
   `01_prepare_masks.py`; it provides the segment-by-exclusion masks used in
   Figure 6, including the combined primary target.
8. Run `08_make_source_profiles.py` with the HCP fixed-effects root, candidate
   ...
   Run the voxelwise-mean sensitivity separately with `--statistic mean` as
   shown below. Median is the default primary estimator. The target table
   separately identifies primary and sensitivity masks; `voxel_statistic_role`
   describes the estimator only. Export checks reject a mean-estimator run
   supplied for primary figures.
...
10. Run `10_export_figure_tables.py` with the completed stage directories to
    create all 31 manuscript-facing tables. Supply the Figure 7, mean sensitivity, Figure S5,
...
| `--mean-source-profiles-dir` | Separate voxelwise-mean sensitivity run from `08_make_source_profiles.py` |
...
Emotion discovery joins four run numbers across all `ses-*` directories.
Task names such as `emotion`, `emotioncb2`, and `emotioncb3` are preserved when
matching cleaned BOLD files to their raw BIDS event files. Each run number 1--4
must occur exactly once per participant, even when acquisitions span sessions.
Cleaned files live under
`derivatives/slabpreproc/sub-*/ses-*/preproc/`; event files live under
`sub-*/ses-*/func/`. The follow-up Emotion data are obtained from the authors;
they are not part of the public movie dataset.
...
Figure 7 and Figure S5 use 1,000 participant bootstrap draws. The Figure S4
...
```bash
uv run python analysis/08_make_source_profiles.py \
  --subjects <task-subjects.tsv> \
  --fixed-effects-root <hcp-fixed-effects-root> \
  --targets <mask-output-dir>/figure7_targets.tsv \
  --candidates <candidate-output-dir>/source_candidates.tsv \
  --gray-matter-support <candidate-output-dir>/gray_matter_support.nii.gz \
  --amygdala-mask <mask-output-dir>/cit168_amygdala_p50.nii.gz \
  --statistic mean \
  --output-dir outputs/analysis/source_profiles_mean
```

The modeled-blur comparison uses median reference normalization and belongs
to the primary run. The mean sensitivity's correlations and intervals are
exported separately by `--mean-source-profiles-dir`.

Smoothing extent is the fraction of supported anatomical-amygdala voxels with
fixed-effects z >= 3, computed within each participant before group aggregation.
Unsupported voxels are excluded from that participant's denominator.
